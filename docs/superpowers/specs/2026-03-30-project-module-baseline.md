...
- `frontend-client` 仍有历史 TypeScript 问题，主要集中在公告、预约、充电记录等旧链路。
...
- `frontend-client` 历史 TypeScript 问题分模块清理
- 车辆模块管理端边界识别
