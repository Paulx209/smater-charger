...
CREATE TABLE `charging_pile` (
    ...
    `status` VARCHAR(20) NOT NULL DEFAULT 'IDLE' COMMENT '实时状态（IDLE-空闲, CHARGING-充电中, WAITING_LEAVE-待驶离, FAULT-故障, OVERTIME-超时占位）',
    `created_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE `charging_record` (
    ...
    `leave_time` DATETIME DEFAULT NULL COMMENT '车辆驶离确认时间',
    `target_type` VARCHAR(20) DEFAULT NULL COMMENT '充电目标类型（DURATION-时长, KWH-电量）',
    `target_value` DECIMAL(10,2) DEFAULT NULL COMMENT '用户输入的充电目标值',
    `target_duration_minutes` INT DEFAULT NULL COMMENT '目标充电时长（分钟）',
    `target_kwh` DECIMAL(10,3) DEFAULT NULL COMMENT '目标充电电量（kWh）',
    `target_end_time` DATETIME DEFAULT NULL COMMENT '目标结束时间',
    `end_reason` VARCHAR(30) DEFAULT NULL COMMENT '结束原因（USER_MANUAL-用户手动, AUTO_TARGET_REACHED-达到目标自动结束）',
    `pre_end_notice_sent` TINYINT NOT NULL DEFAULT 0 COMMENT '结束前提醒是否已发送',
    ...
    `status` VARCHAR(20) NOT NULL DEFAULT 'CHARGING' COMMENT '订单状态（CHARGING-充电中, COMPLETED-已完成, CANCELLED-已取消）',
    `created_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_user_id` (`user_id`),
    ...
    KEY `idx_target_end_time` (`target_end_time`),
...
CREATE TABLE `warning_notice` (
    ...
    `type` VARCHAR(50) NOT NULL COMMENT '通知类型（IDLE_REMINDER-空闲提醒, OVERTIME_WARNING-超时占位预警, FAULT_NOTICE-故障通知, RESERVATION_REMINDER-预约提醒, CHARGING_ENDING_SOON-即将结束, CHARGING_COMPLETED-充电完成）',
